...
import DocExtractor from './doc-extraction'
...
export default class Analysis {
  ...
  _onCreateNode = (node) => {
    ...
    this.docExtractor.onCreateNode(node)
  ...
  _analyseModule = async (moduleData) => {
    ...
    try {
      ...
      analysis.docExtractor.processNodes()
    } catch (ex) {
  ...
  solveQuery = async () => {
    return await this._evaluate(() => {
      ...
      this.docExtractor.processNodes()

      let queryType = this.queryType
      if (this.queryBase) {
        queryType = nodeGetType(this.queryBase)
        this.docExtractor.addDescriptionsToQueryType(queryType, this.queryBase.name)
      }
      ...
      if( ! this.queryBase ) {
        this.docExtractor.addDescriptionsToResults( results )
      }
