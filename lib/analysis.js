...
export default class Analysis {
  ...
  _onCreateNode = (node) => {
    ...
    if (
      ...
      (node.base.name === 'require' || node.base.name === 'import')
    ) {
  ...
  /**
   * Returns identifier for node at index i within nodes.
   */
  _getIdentifier( nodes, i, qualified ) {
      let node = nodes[i]
      switch( node.type ) {
        case 'FunctionDeclaration':
          if( ! node.identifier ) {
            // 'a.b.c = function()' form, backtrack to get identifier form AssignmentStatement
            for( let j=i-1; j>=0; j-- ) {
              if( backNode.type === 'AssignmentStatement' ) {
                return this._getIdentifier(nodes, j, qualified)
              }
            }
            return ''
          }
          ...
          return node.identifier.identifier.name
        ...
        case 'AssignmentStatement':
          ...
          return qualified
            ? variable.base.name + '.' + variable.identifier.name
  ...
  _addDescriptionsToQueryType( queryType ) {
    ...
    for( let fieldName in fields ) {
      ...
      if( ! field.description ) {
        field.description = this._getDescription(fieldName)
      }
    }
  }
    ...
    for( let node of nodes ) {
      switch( node.type ) {
        ...
        case 'AssignmentStatement':
          ...
          this._setDescription( this._getIdentifier(nodes, i, true), Analysis.lastComment )
          this._setDescription( this._getIdentifier(nodes, i, false), Analysis.lastComment )
